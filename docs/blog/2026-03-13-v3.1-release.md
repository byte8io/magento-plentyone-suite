...
This release addresses real-world issues reported by clients and introduces long-awaited customer group mapping support during order export. It also includes the official rebrand from Byte8 to Byte8 Ltd across all modules.
...
All modules have been updated to reflect the company rebrand from **Byte8 Ltd** to **Byte8 Ltd**. This includes:

- Copyright headers updated across all source files
- License files updated with new company details and URLs
- Guide URLs updated from `byte8.io` to `byte8.io`
...
```bash
# Update via Composer
composer require byte8/magento-plentyone-suite:^3.1
