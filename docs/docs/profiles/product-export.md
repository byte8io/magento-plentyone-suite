...
bin/magento softcommerce:profile:history:list --profile-id=6

# View last 10 executions
bin/magento softcommerce:profile:history:list --profile-id=6 --limit=10

# Clear profile history
bin/magento softcommerce:profile:history:clear --profile-id=6
```
...
3. **Add Products to Queue**
   ...
   bin/magento softcommerce:plenty:item:queue:add --profile-id=6
   ```

4. **Test Export with Small Batch**
   ```bash
   # Export first 10 products
   bin/magento softcommerce:plenty:item:export --profile-id=6 --batch-size=10 --limit=10
   ```
...
6. **Full Export**
   ```bash
   # Export all products
   bin/magento softcommerce:plenty:item:export --profile-id=6 --batch-size=20
   ```
...
```bash
# Check queue status
bin/magento softcommerce:plenty:item:queue:list --profile-id=6 --status=pending

# Check for errors
bin/magento softcommerce:plenty:item:queue:list --profile-id=6 --status=error
...
bin/magento softcommerce:plenty:item:queue:add --profile-id=6 --updated-since="1 hour ago"
...
2. **Add to Queue**
   ```bash
   bin/magento softcommerce:plenty:item:queue:add --profile-id=6 --sku=TSHIRT-PARENT
   ```

3. **Export**
   ```bash
   bin/magento softcommerce:plenty:item:export --profile-id=6 --batch-size=5
   ```
...
6. **Test with Single Configurable**
   ```bash
   bin/magento softcommerce:plenty:item:export --profile-id=6 --sku=CONFIGURABLE-SKU --batch-size=1
   ```
...
7. **Test Image Export**
   ...
   bin/magento softcommerce:plenty:item:export --profile-id=6 --sku=TEST-SKU -vvv
   ```
...
2. **Increase PHP Memory Limit**
   ...
   # Or via CLI
   php -d memory_limit=2G bin/magento softcommerce:plenty:item:export
   ```

3. **Export by Product Type**
   ```bash
   # Export simple products first (small batches)
   bin/magento softcommerce:plenty:item:export --profile-id=6 --type=simple --batch-size=30

   # Then configurable (smaller batches)
   bin/magento softcommerce:plenty:item:export --profile-id=6 --type=configurable --batch-size=5
   ```
...
6. **Monitor Memory Usage**
   ...
   php -d memory_limit=2G bin/magento softcommerce:plenty:item:export --profile-id=6 -vvv
   ```
...
2. **Increase Batch Size**
   ...
   bin/magento softcommerce:plenty:item:export --profile-id=6 --batch-size=50
   ```
...
4. **Parallel Processing**
   ...
   bin/magento softcommerce:plenty:item:export --profile-id=6 --type=simple &

   # Terminal 2: Process configurable products
   bin/magento softcommerce:plenty:item:export --profile-id=6 --type=configurable &
   ```

5. **Clear Old Completed Items**
   ```bash
   bin/magento softcommerce:plenty:item:queue:clear --profile-id=6 --status=complete
   ```
...
7. **Identify Problematic Products**
   ...
   bin/magento softcommerce:plenty:item:queue:list --profile-id=6 --status=processing
...
1. **Export by Product Type**
   ...
   bin/magento softcommerce:plenty:item:export --type=simple --batch-size=30

   # Day 2: Configurable products (slower)
   bin/magento softcommerce:plenty:item:export --type=configurable --batch-size=10

   # Day 3: Bundle products
   bin/magento softcommerce:plenty:item:export --type=bundle --batch-size=10
   ```
...
1. **Daily Monitoring**
   ```bash
   # Check queue status
   bin/magento softcommerce:plenty:item:queue:list --profile-id=6 --status=pending | wc -l

   # Check errors
   bin/magento softcommerce:plenty:item:queue:list --profile-id=6 --status=error

   # View recent history
   bin/magento softcommerce:profile:history:list --profile-id=6 --limit=5
   ```
